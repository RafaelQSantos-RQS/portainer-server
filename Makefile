...
.PHONY: help setup up down restart sync status logs pull _check-env-exists _create-env-from-template _create-network-if-not-exists _create-volume-if-not-exists
...
_create-volume-if-not-exists:
	@echo "==> Checking for volume $(SERVER_DATA_VOLUME_NAME)..."
	@docker volume inspect $(SERVER_DATA_VOLUME_NAME) >/dev/null 2>&1 || \
		(echo "==> Volume $(SERVER_DATA_VOLUME_NAME) not found. Creating..." && docker volume create $(SERVER_DATA_VOLUME_NAME))
	@echo "✅ Volume $(SERVER_DATA_VOLUME_NAME) is ready."
...
validate: ## ✅ Validate the Traefik configuration file syntax.
